...
def main(args):
    ...
    for image_file in image_file_list:
        ...
        for i in range(0, len(dt_boxes)):
            result.append({
                "text": rec_res[i][0],
                "confidence": rec_res[i][1],
                "text_box_position": dt_boxes[i].tolist()
            })
...
def analyse_image_web(args):
    result = list()
    ########## 读取base64图片 ##########
    encode_data = args.base64Img
    ...
    if not os.path.exists(draw_img_save):
        os.makedirs(draw_img_save)
    cv2.imwrite(
        os.path.join(draw_img_save, 'flask-ocr-test-file.png'),
        draw_img[:, :, ::-1])
    logger.info("The visualized image saved in {}".format(
        os.path.join(draw_img_save, 'flask-ocr-test-file.png')))
